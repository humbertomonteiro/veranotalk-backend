...
class ParticipantController {
    ...
    async updateParticipant(req, res) {
        try {
            ...
            const { name, email, phone, ticketType, status, paymentMethod, totalAmount, repo = "both", } = req.body;
            if (!name &&
                !email &&
                !phone &&
                !ticketType &&
                ...
                res.status(400).json({
                    error: "Pelo menos um campo deve ser fornecido (name, email, phone, ticketType, status, paymentMethod, totalAmount)",
                });
                return;
            }
            const result = await this.participantService.updateParticipant(uid, {
                name,
                email,
                phone,
                ticketType,
            ...
            console.error(`Erro ao atualizar participante/checkout com UID ${req.params.uid}:`, error);
            res.status(400).json({
